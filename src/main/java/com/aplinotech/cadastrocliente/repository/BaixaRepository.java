...
public interface BaixaRepository extends JpaRepository<Baixa, Long> {
    ...
    @Query(value="SELECT i FROM ItemBaixa i WHERE baixa.data BETWEEN :dataInicio AND :dataFim AND i.usuario.id = :idUsuario")
    List<ItemBaixa> findByDates(@Param("dataInicio") Date dataInicio,
